...
class EventsFlow extends Flow
  ...
  remove_from_timeline: (object, event_type) =>
    ...
    timeline_events = if Events\object_type_for_object(object) == Events\object_type_for_model(Users)
      ...
      db.select "* from timeline_events join events on timeline_events.event_id = events.id and events.source_user_id = ?", object.id
    else 
      -- If we are removing the subscription from Module
      db.select "* from timeline_events join events on timeline_events.event_id = events.id and events.source_user_id = ? and events.object_type = ? and object_id = ?", object.user_id, Events\object_type_for_object(object), object.id
